...
//                String txt=event.getClipDescription().getLabel().toString();
        ...
        if (mLines != null) {
            for (Line line : mLines) {
                ...
                for (Item item : items) {
                    ...
                    } else if (item.isSpaceSymbol()) {
                        (item.view).setPadding(mSpaceSymbolTextPadding, mTextPaddingPort, mSpaceSymbolTextPadding, mTextPaddingPort);
                    } else {
    ...
    public void setSpaceSymbolTextPadding(int padding) {
        mSpaceSymbolTextPadding = padding;
        mNeedReDetectInMeasure = true;
        if (mLines != null) {
            for (Line line : mLines) {
                List<Item> items = line.getItems();
...
//        if (TextUtils.isEmpty(text) || text.equals(" ")) {
        ...
        } else if (StringUtil.isSpace(text)) {
            view.setPadding(mSpaceSymbolTextPadding, mTextPaddingPort, mSpaceSymbolTextPadding, mTextPaddingPort);
        } else {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mNeedReDetectInMeasure) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                child.getBackground().setAlpha(210);
                ...
                if (currentLine.getItems() == null && (isEnter && showSection)) {
                    ...
                    // 20240119 被选中的非空白符号也显示，查看符号见SYMBOL_REX_WITHOUT_BLANK
                    if (isBold) {
                        item.setSelected(true);
                    }
                } else if (item.isSpaceSymbol()) {
...
//                    child.setPadding(mSymbolTextPadding, mTextPaddingPort, mSymbolTextPadding, mTextPaddingPort);
                    child.setPadding(mSpaceSymbolTextPadding, mTextPaddingPort, mSpaceSymbolTextPadding, mTextPaddingPort);
...
//                    dragItem = null;
            ...
            switch (actionMasked) {
                case MotionEvent.ACTION_DOWN:
                    if (touchTime == null) touchTime = System.currentTimeMillis();
...
//                item.view.setSelected(true);
    ...
    private LongPressedSelectionRunnable mLongPressedSelectionRunnable = new LongPressedSelectionRunnable();

    private class LongPressedSelectionRunnable implements Runnable {
        int x, y;
    ...
    private void multiPressed(int x, int y) {
        Item item = findItemByPoint(x, y);
        if (item == null) return;

        List<Item> itemList = getItemAll();
        if (mStartItem == null) {
            mStartItem = item;
            return;
        }
        int start = itemList.indexOf(mStartItem);
        int end = itemList.indexOf(item);
        if (start == end || start == -1 || end == -1)
            return;
        if (isSelected == null) {
            isSelected = start > end ? !item.isSelected() : item.isSelected();
        }
        ...
        if (item != mStartItem) {
            ...
            for (int i = start; i != end; i = i + step) {
                if (itemList.get(i).isSelected() != toSelect)
                    itemList.get(i).setSelected(toSelect);
            }
            if (itemList.get(end).isSelected() != toSelect)
                itemList.get(end).setSelected(toSelect);
        }
        mStartItem = mTargetItem;
    }
...
//                item.setSelected(!item.isSelected());
    ...
    public List<Item> getItemAll() {
        return mLines.stream()
                .flatMap(line -> line.getItems().stream())
    ...
    public class Item {
        ...
        public boolean isSpaceSymbol() {
            return StringUtil.isSpace(((TextView) view).getText().toString());
        }
            ...
            if (isSymbolSelectable() || (!isSymbol() && !isSpaceSymbol())) {
                ...
                if (selected) {
                    ...
                    view.animate()
                            .scaleX(0.9f) // 放大X轴
                            .scaleY(0.9f) // 放大Y轴
                            .setDuration(150) // 设置动画持续时间
                            .withLayer() // 启用硬件加速
                            .setInterpolator(new OvershootInterpolator()) // 设置弹簧效果的插值器

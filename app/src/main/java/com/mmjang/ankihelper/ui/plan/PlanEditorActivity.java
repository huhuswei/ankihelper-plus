...
//        }
        ColorThemeUtils.initColorTheme(PlanEditorActivity.this);
...
//        }
    ...
    private void populateDictionary() {
        ...
        if (planForEdit != null) {
            ...
            if(!find){
            String message = String.format(getResources().getString(R.string.str_no_dictionary), key1);
    ...
    private void refreshFieldSpinners() {
        ...
        String[] allElements = Utils.concatenate(dictionaryElements, sharedElements);
        ...
        if (planForEdit != null && currentModelId == planForEdit.getOutputModelId()) {
            for (String fld : fields) {
                LinkedHashMap<String, String> fldMap = planForEdit.getFieldsMap();
                if (fldMap.containsKey(fld)) {
                    ComplexElement ce = new ComplexElement(fldMap.get(fld));
                    String[] planAllElements = Utils.concatenate(ce.getElementNormal().split(MultiSpinner.DELIMITER), ce.getElementAppending().split(MultiSpinner.DELIMITER));
                    boolean isExist = true;
                    for (String planElement : planAllElements) {
                        isExist = Arrays.stream(allElements).anyMatch(n->planElement.equals(n));
                        if (!isExist)
                            break;
                    }
                    if (isExist) {
                        fieldsMapItemList.add(new FieldsMapItem(fld, allElements, ce.getElementNormal(), ce.getElementAppending()));
                    } else {
...
//                    fieldsMapItemList.add(new FieldsMapItem(fld, allElements, ce.getElementNormal(), ce.getElementAppending()));
    ...
    private boolean savePlan() {
        Trace.i("Model_currentDeckId", Long.toString(currentDeckId));
        Trace.i("Model_currentModelId", Long.toString(currentModelId));
...
//            String av = item.getExportedElementNames()[item.getSelectedFieldAppendingPos()];
            String nv = item.getMultiSel();
            String av = item.getMultiSelAppending();
        ...
        String modelName = mAnkiDroid.getApi().getModelName(plan.getOutputModelId());
        if(!StringUtils.isEmpty(modelName) && StringUtils.isEmpty(modelsMap.get(plan.getOutputModelId()))) {
            modelsMap.put(plan.getOutputModelId(), modelName);
        }
